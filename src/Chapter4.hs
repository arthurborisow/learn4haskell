...
concatL Empty = Empty
concatL (Cons x Empty) = x
concatL (Cons x (Cons y y')) = addLists x (addLists y (concatL y'))
...
andM l r = l >>= \l' -> if l' then r else pure False
-- this one feels right for some reason but fails tests
-- andM l r = l >>= \l' -> fmap (l' &&) r
